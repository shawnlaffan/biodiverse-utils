...
foreach my $count (10, 50, 100, 300, 500, 1000) {
    ...
    for my $idx_start (0 .. 99) {
        my @shuffled_labels = $prng4->shuffle (@labels);
        
        my @sublist = @shuffled_labels[0 .. $check_count];
        push @sublist, @labels[0..$check_count];  #  make sure we have some overlap between lists
    ...
    #is_deeply ($xs, $grep_first, "xs and grep_first match");
    #is_deeply ($xs, $next_if, "xs and next_if match");
    #is_deeply ($xs, $hash_crash, "xs and hash_crash match");
    #is_deeply ($xs, $pp, "xs and pp match");
    ...
    cmpthese (
        -3,
        {
            ...
            xs         => sub {xs(\@key_hashes)},
            pp         => sub {pp(\@key_hashes)},
        }
    );
}
...
sub xs {
    ...
    for my $sub_list (@$key_lists) {
        Biodiverse::Utils::XS::add_hash_keys (\%path, $sub_list);
    }
...
sub pp {
    ...
    for my $sub_list (@$key_lists) {
        Biodiverse::Utils::PP::add_hash_keys (\%path, $sub_list);
    }
