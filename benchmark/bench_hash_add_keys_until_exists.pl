...
foreach my $count (10, 50, 100, 300, 500, 1000) {
    ...
    #  generate some new labels and prepend them as new lists
    #  idx_start is where the lists should begin to overlap
    for my $idx_start (1 .. $count) {
        my $start = $max + 1;
        my $end   = $max + $idx_start;
        my @sublist = ($start .. $end);
        push @sublist, @labels[$idx_start..$#labels];
