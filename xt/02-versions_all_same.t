...
my $wanted = sub {
    ...
    $filename =~ s/\.pm$//;
    $filename =~ s{/}{::}g;
    if ($filename =~ /lib::(Biodiverse.*)$/) { #  get the package part - very clunky
        $filename = $1;
    }
...
require Biodiverse::Utils;

my $version = $Biodiverse::Utils::VERSION;

note ( "Testing Biodiverse::Utils $version, Perl $], $^X" );

my $blah = $Biodiverse::Utils::VERSION;
